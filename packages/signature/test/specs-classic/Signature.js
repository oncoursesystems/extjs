topSuite('Ext.signature.Signature', ['Ext.signature.*', 'Ext.Container'], function() {
    ...
    afterEach(function() {
        Ext.destroy(container, signature);
    ...
    describe('Dynamic updates', function() {
        ...
        it('should update background color', function() {
            ...
            // Spy on the clear method to ensure it is called when background color changes
            spyOn(sp, 'clear');
    ...
    describe('SignaturePad event firing with pointer events', function() {
        ...
        it('should fire `signaturestart` and `signatureend` via pointer event simulation', function() {
            ...
            waitsFor(function() {
                return signature.getSignaturePad() && signature.el && signature.canvas;
            });

            runs(function() {
                var canvas = signature.canvas.dom,
                    // Simulate a triangle stroke on canvas
                    pointA = { x: 100, y: 100 },
                    pointB = { x: 150, y: 50 },
                    pointC = { x: 200, y: 100 };
                ...
                function dispatch(type, x, y, options) {
                    ...
                    Ext.apply(eventOptions, options);
                    canvas.dispatchEvent(new PointerEvent(type, eventOptions));
                }

                dispatch('pointerdown', pointA.x, pointA.y);
                dispatch('pointermove', pointB.x, pointB.y);
                dispatch('pointermove', pointC.x, pointC.y);
                dispatch('pointermove', pointA.x, pointA.y);
                dispatch('pointerup', pointA.x, pointA.y);
            });
    ...
    describe('Destroy', function() {
        it('should clean up signaturePad and listeners on destroy', function() {
            createSignaturePad();
