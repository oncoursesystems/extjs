...
Ext.define('Ext.qrcode.generator.GeneratorBase', {
    ...
    renderTpl: [
        '<canvas id="{id}-canvasEl" data-ref="canvasEl" ' + 'class="qr-canvas" width="{qrSize}" height="{qrSize}"></canvas>'
    ],
    renderSelectors: {
...
Ext.define('Ext.qrcode.generator.QRCode', {
    ...
    generate: function(data, type) {
        ...
        for (i = 0; i < moduleCount; i++) {
            for (j = 0; j < moduleCount; j++) {
                ...
                ctx.fillRect(Math.round(j * tileW), Math.round(i * tileH), Math.ceil(tileW), Math.ceil(tileH));
            }
        }
    ...
    */
    copyToClipboard: function() {
        ...
        hasClipboardSupport = navigator.clipboard && Ext.isFunction(ClipboardItem) && Ext.isFunction(navigator.clipboard.write);
    ...
    downloadAsPNG: function() {
        var me = this,
            ...
            link, filename;
        if (canvas) {
            Ext.Msg.prompt('Download As PNG', 'Enter filename (without extension):', function(btn, text) {
                if (btn === 'ok' && text) {
                    filename = text.trim();
                    if (!filename) {
    ...
    downloadAsSVG: function() {
        ...
        if (qr && canvas) {
            Ext.Msg.prompt('Download SVG', 'Enter filename (without extension):', function(btn, text) {
                if (btn === 'ok' && text) {
                    filename = text.trim();
                    filename = filename.replace(/[<>:"/\\|?*]/g, '_');
                    svg = qr.createSvgTag({
                    ...
                    svg = svg.replace(/<svg([^>]*?)>/, function(match, attrs) {
                        ...
                        return '<svg width="' + canvas.width + '" height="' + canvas.height + '"' + cleanedAttrs + '>';
                    });
                    blob = new Blob([
                        svg
                    ], {
    ...
    getWIFIQRData: function(wifiObj) {
        return 'WIFI:T:' + wifiObj.encryptionType + ';S:' + wifiObj.ssid + ';P:' + wifiObj.password + ';';
    },
    /**
     * @private
     */
    ...
    getUPIQRData: function(upiObj) {
        var upiLink = 'upi://pay?pa=' + encodeURIComponent(upiObj.vpa) + '&pn=' + encodeURIComponent(upiObj.name) + '&am=' + encodeURIComponent(upiObj.amount) + '&cu=' + upiObj.currency;
        return upiLink;
    },
    /**
     * @private
    ...
    getBitcoinQRData: function(bitCoinObj) {
        return 'bitcoin:' + bitCoinObj.address + '?amount=' + bitCoinObj.amount + '&label=' + encodeURIComponent(bitCoinObj.label);
    },
    /**
     * 
...
Ext.define('Ext.qrcode.reader.QRCodeReader', {
    ...
    scannerWindow: {
        ...
        items: [
            {
                xtype: 'component',
                html: '<div id="qr-scanner-preview" ' + 'style="position: relative; width: 100%; height: 100%;"></div>'
            }
        ]
    },
    /**
    ...
    startScanner: function(scanButton) {
        ...
        Html5Qrcode.getCameras().then(function(cameras) {
            ...
            return me.qrScanner.start(cameraId, me.getScannerConfig(), function(qrCodeMessage) {
    ...
    scanQRFile: function(qrCodeFile, qrComponent) {
        ...
        if (window.Html5Qrcode) {
            ...
            html5QrCode.scanFile(qrCodeFile, true).then(function(decodedText) {
    ...
    stopScanner: function() {
        ...
        try {
            me.qrScanner.stop().then(function() {
