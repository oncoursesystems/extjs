...
Ext.onReady(function() {
    var employeeStore, radarStore, reviewStore;
    ...
    employeeStore = new Ext.data.Store({
    ...
    Ext.define('App.RadarStore', {
            ...
            Ext.Array.each(reviews, function(review, recordIndex, all) {    // add a new radarStore record for each review record 
                ...
                review.fields.each(function(field) {
                    ...
                    if (field.name !== "employee_id" && field.name !== "review_date") {  // filter out the fields we don't need
                        metricRecord = me.findRecord('metric', field.name); // checks for an existing metric record in the store

                        if (metricRecord) {
                            metricRecord.set(eid, review.get(field.name));  // updates existing record with field value from review
                        }
                        else {
                            newRecord = {};    // creates a new object we can populate with dynamic keys and values to create a new record
        ...
        calculateAverageScores: function() {
            var me = this, // keeps the store in scope during Ext.Array.each
                ...
                Review = Ext.ModelMgr.getModel('Review'),
                avgScore, record;
            ...
            Ext.Array.each(Review.prototype.fields.keys, function(fieldName) {  // loop through the Review model fields and calculate average scores
                if (fieldName !== "employee_id" && fieldName !== "review_date") {  // ignore non-score fields
                    avgScore = Math.round(reviewStore.average(fieldName));  // takes advantage of Ext.data.Store.average()
                    record = me.findRecord('metric', fieldName);
                ...
                buttons: [{
                    ...
                    handler: function(btn, eventObj) {
                        var window = btn.up('window'),
                            ...
                            vals, employeeStore, currentEmployee, currentReview, newId;

                        if (form.isValid()) {
                            window.getEl().mask('saving data...');
                            vals = form.getValues();
                            employeeStore = Ext.data.StoreMgr.lookup('employeeStore');
                            currentEmployee = employeeStore.findRecord('id', vals.employee_id);
                            ...
                            if (vals.employee_id && currentEmployee) {
                                ...
                                currentReview = Ext.data.StoreMgr.lookup('reviewStore').findRecord('employee_id', vals.employee_id);
                                ...
                                newId = employeeStore.getCount() + 1;
    ...
    function refreshRadarChart(employees) {
        var existingRadarChart = Ext.getCmp('radarchart'), // grab the radar chart component (used down below)
            reportsPanel = Ext.getCmp('reportspanel'), // grab the reports panel component (used down below)
            dynamicSeries = [], // setup an array of chart series that we'll create dynamically
            index, fullName, eid, newRadarChart;
        ...
        for (index = 0; index < employees.length; index++) {
            fullName = employees[index].get('first_name') + ' ' + employees[index].get('last_name');
            eid = 'eid_' + employees[index].get('id');
        ...
        newRadarChart = new App.PerformanceRadar({ series: dynamicSeries });
    ...
    function refreshEmployeeDetails(employees) {
        var detailsPanel = Ext.getCmp('detailspanel'),
            reviewStore = Ext.data.StoreMgr.lookup('reviewStore'),
            items = [],
            index, templateData, employeePanel;

        for (index = 0; index < employees.length; index++) {
            templateData = Ext.applyIf(employees[index].data, reviewStore.findRecord('employee_id', employees[index].get('id')).data);
            employeePanel = new App.EmployeeDetail({
                        ...
                        items: [
                            {
                                ...
                                handler: function(grid, rowIndex, colIndex) {
                                    var employee = grid.getStore().getAt(rowIndex),
                                        review = reviewStore.findRecord('employee_id', employee.get('id')),
                                        win = new App.ReviewWindow({ hidden: true }),
                            ...
                            },
                            {
                                ...
                                handler: function(grid, rowIndex, colIndex) {
                                    Ext.Msg.confirm(
                                        'Remove Employee?', 'Are you sure you want to remove this employee?', function(choice) {
                                            var reviewStore, employee, reviewIndex;

                                            if (choice === 'yes') {
                                                reviewStore = Ext.data.StoreMgr.lookup('reviewStore');
                                                employee = grid.getStore().getAt(rowIndex);
                                                reviewIndex = reviewStore.find('employee_id', employee.get('id'));
